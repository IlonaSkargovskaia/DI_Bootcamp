...
-- SELECT COUNT(emplname)
-- FROM EMPLOYEE WHERE Department_code = 10

-- How many employees are analyst in dept 10
-- SELECT COUNT(emplname)
-- FROM EMPLOYEE
-- WHERE Department_code = 10 AND job ILIKE '%ANALYST%'

-- Display the names of each employees, their job and dept location
-- SELECT EMPLOYEE.emplname, EMPLOYEE.job, DEPARTMENT.LOCATION
-- FROM EMPLOYEE
...
-- Create a query that displays EMPFNAME, EMPLNAME, Department_code, 
-- DEPTNAME, LOCATION from EMPLOYEE, and DEPARTMENT tables. 
-- Make sure the results are in the ascending order based on the 
-- EMPFNAME and LOCATION of the department.

-- SELECT EMPFNAME, EMPLNAME, Department_code, DEPARTMENT.DEPTNAME, DEPARTMENT.LOCATION
-- FROM Employee
-- INNER JOIN DEPARTMENT
-- ON DEPARTMENT.DEPTCODE = EMPLOYEE.Department_code
...
-- SELECT empfname, commission + salary AS total_salary
-- FROM employee
