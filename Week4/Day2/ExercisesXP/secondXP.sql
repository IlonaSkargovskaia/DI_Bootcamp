...
-- Write a query to get the film ID, title, description, length and the rental rate of all the movies starting with 
...
-- SELECT title,rental_rate FROM film
-- ORDER BY rental_rate ASC OFFSET 10 LIMIT 10 

-- Write a query which will join the data in the 
-- customer table and the payment table. You want to get the 
-- first name and last name from the curstomer table, as well as the 
-- amount and the date of every payment made by a customer, ordered by 
-- their id (from 1 to…).
...
-- Bonus You want to be able to see how your 
-- sellers have been doing? Write a query to get 
-- the customer’s id, names (first and last), the amount 
